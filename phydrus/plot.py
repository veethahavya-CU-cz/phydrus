...
class Plots:
    ...
    @staticmethod
    def _add_h0_line(ax, axis="x"):
        """Draw a subtle reference line at h=0 on the selected axis."""
        if axis == "x":
            ax.axvline(0.0, color="gray", lw=0.8, alpha=0.8)
        elif axis == "y":
            ax.axhline(0.0, color="gray", lw=0.8, alpha=0.8)

    def profile(self, figsize=(3, 6), title=None, cmap="YlOrBr", color_by="Ks", show_grid=True, **kwargs):
        """
        ...
        h_max = self.ml.profile.loc[:, "h"].max(skipna=True)
        h_min = self.ml.profile.loc[:, "h"].min(skipna=True)
        if pd.isna(h_max):
            h_max = 0
        if pd.isna(h_min):
            h_min = 0
        x_right = h_max + 0.2 * abs(h_max)
        x_left = min(0.0, h_min * 1.1)
        w = x_right - x_left  # total width of each material rectangle
        ...
        for i in self.ml.profile.index[1:]:
            ...
            patch = plt.Rectangle(
                xy=(x_left, top), width=w, height=h, linewidth=1, edgecolor=edgecolor, facecolor=color
            )
            ax.add_patch(patch)
            top = bot

        line = ax.plot(self.ml.profile.loc[:, ["h"]].values, self.ml.profile.loc[:, ["x"]].values, label="Initial head")
        self._add_h0_line(ax, axis="x")
        ...
        for i, color in enumerate(colors):
            legend_elements.append(plt.Rectangle((0, 0), 0, 0, color=color, label=f"material {i}"))

        plt.legend(handles=legend_elements, loc="best")
        plt.tight_layout()
        return ax

    def profile_information(self, data="Pressure Head", times=None, legend=True, figsize=(5, 3), **kwargs):
        """
    ...
    def water_flow(self, data="Potential Surface Flux", figsize=(6, 3), **kwargs):
        ...
        cols = ("rTop", "rRoot", "vTop", "vRoot", "vBot", "hTop", "hRoot", "hBot", "RunOff", "Volume")
        df = self.ml.read_tlevel()
        col = col_names.index(data)
        h_cols = {"hTop", "hRoot", "hBot"}
        ...
        if col < 5:
            ...
            if cols[col] in h_cols:
                self._add_h0_line(axes[0], axis="y")
                self._add_h0_line(axes[1], axis="y")
        else:
            if data == "Surface runoff":
                _, axes = plt.subplots(1, 2, figsize=figsize, **kwargs)
                df.plot(y="RunOff", ax=axes[0], use_index=True)
                axes[0].set_ylabel(data)
                axes[0].set_xlabel(f"Time [{self.ml.basic_info['TUnit']}]")

                # Cumulative runoff from T_LEVEL.OUT
                if "sum(RunOff)" in df.columns:
                    df.plot(y="sum(RunOff)", ax=axes[1], use_index=True)
                else:
                    # Fallback for output configurations where cumulative runoff
                    # is not explicitly written to T_LEVEL.OUT.
                    axes[1].plot(df.index, df["RunOff"].cumsum(), label="sum(RunOff)")
                axes[1].set_ylabel(f"Cum. {data}")
                axes[1].set_xlabel(f"Time [{self.ml.basic_info['TUnit']}]")
            else:
    ...
    def soil_properties(self, data="Water Content", figsize=(6, 3), **kwargs):
        ...
        # Left panel uses h on x-axis for all variables.
        self._add_h0_line(axes[0], axis="x")

        # For Pressure head, h is also on the y-axis.
        if cols[col] == "h":
            self._add_h0_line(axes[0], axis="y")
            self._add_h0_line(axes[1], axis="y")
